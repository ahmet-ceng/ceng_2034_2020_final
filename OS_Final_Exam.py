...
# n equals to 0 means child process 
	...
	for i in url_list:
		...
		#Determining the type of the url.
		#There are probably much more easier and accurate ways to find the image format of the url but this is the solution I came up with.
...
#***I created 2 ways.First without multiprocessing than with multiprocessing.Multiprocessing way does not delete the files it only finds them***
#***Reason I created 2 ways is first I forget to implement multiprocessing :D (detailed explanation in my report)***

        #This function finds duplicates.(used in multiprocessing way.)
	def duplicatefinder(hash1, hash2):
		if hash1[0] != hash2[0]:
			if hash1[1] == hash2[1]:
				print(hash1[0], "=", hash2[0])#duplicates
	...
	which_way = input("There are two ways to find duplicates.Enter '1' for normal,Enter '2' for multiprocessing way.")
	...
	if which_way == "2":
                ...
                new_list_of_photos = []
                a=0	
                for item in list_of_photos:
                        b = location+list_of_photos[a]
                        a+=1
                        new_list_of_photos.append((item, hashfinder(b)))
                
                print(new_list_of_photos,"\n")
                pool = Pool()
                pool.starmap(duplicatefinder, product(new_list_of_photos, repeat = 2))
